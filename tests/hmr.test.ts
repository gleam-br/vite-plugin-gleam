...
describe("HMR and Error Handling", () => {
  ...
  it("should invalidate affected modules on gleam file change", async () => {
    // Mock projectBuild before buildStart so tests don't require external gleam binary in CI
  ...
  it("should report compiler errors to Vite overlay when build fails", async () => {
    // Initially mock buildStart to succeed
    const buildSpy = vi.spyOn(projectModule, "projectBuild").mockResolvedValue({
      stdout: "",
      stderr: "",
      durationMs: 10,
    });
    ...
    // Now mock projectBuild failure for the hot update
